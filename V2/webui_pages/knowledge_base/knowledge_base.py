    ...
    if selected_kb == "新建知识库":
        ...
        if submit_create_kb:
            if not kb_name or not kb_name.strip():
                ...
                st.session_state["selected_kb_info"] = kb_info
                st.rerun()
    # elif selected_kb == "coalchat":
    #     st.session_state["selected_kb_info"] = kb_list[selected_kb]['kb_info']
    #     kb_info = st.text_area("请输入知识库介绍:", value=st.session_state["selected_kb_info"], max_chars=None,
    #                            key=None,
    #                            help=None, on_change=None, args=None, kwargs=None)
    #     if kb_info != st.session_state["selected_kb_info"]:
    #         st.session_state["selected_kb_info"] = kb_info
    #         api.update_kb_info(selected_kb, kb_info)

    elif selected_kb == "coalchat1":
        st.session_state["selected_kb_info"] = kb_list[selected_kb]['kb_info']
        st.markdown("""
        <hr><center>
        <h3>知识库介绍</h3>
        </center><hr>
        <p>本知识库专为煤矿领域量身打造。它汇集了丰富的语料资源，覆盖了煤矿产业的多个关键领域，包括但不限于安全规程、生产技术、环境保护、法律法规以及设备维修等各个方面。</p>
        <p><strong>如需了解本知识库详细信息，请联系研发团队。</strong></p>""", unsafe_allow_html=True)
